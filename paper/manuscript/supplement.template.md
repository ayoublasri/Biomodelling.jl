...
| 4 | resistance gene | as Fig. 3 with $(k_{\mathrm{on}}, k_{\mathrm{off}}) = (0.002, 0.02)$ (memory, $p_{\mathrm{on}} \approx 9\%$) or $(0.05, 0.5)$ (fast, same $p_{\mathrm{on}}$); founders burnt in for 300 time units at constant size; death $h_{\max} = 0.15$, $\mathrm{EC}_{50} = 0.5$, $m = 2$, $K = 150$, $q = 4$; drug from $t = 40$ of the treatment stage; drug-induced variant $(0.0002, 0.02)$ with $k_{\mathrm{on}} \to k_{\mathrm{on}}(1 + 100 d)$; fitness cost variant: growth $\times (1 - 0.5\, c_P^4/(150^4 + c_P^4))$; memory disruption: switching $\times 20$ on $[0, 40)$ or $[0, 120)$; schedule scan: 300 founders, 240 time units, 20-time-unit exposures; cycle-gated variant (Supplementary Fig. 5) adds $\beta = 0.25$, $\varphi_0 = 0.5$, $w = 0.15$ to the death hazard |
...
| Dose-dense temozolomide does not improve survival | Clinical result [@gilbert2013] | A mechanistic account for unmethylated tumours; with MGMT consumption the model instead predicts a dose-dense advantage in the methylated population, a testable divergence from the trial, and points to the rate at which the drug consumes MGMT as the measurement that separates the regimens |
