...
| 8e-g | MGMT model | same expression kinetics; net doubling 40 days, memory 4 generations, MGMT-expressing fraction 0.01 or 0.30, $h_{\max} = 0.03$ per h at peak, $\mathrm{EC}_{50} = 0.4$ of the standard bolus peak, $m = 2$, $K = 150$, $q = 4$; elimination half-life 2.1 h; 1 500 founders, six 28-day cycles, dt 1 h |

# S4. Additional persister simulations

![](figS1_timing.png)

**Figure S1. Single-cell timing distributions underlying Figure 4c.** (a) Division times of cells born under drug and (b) times from drug start (or from birth, for cells born under drug) to death, per dose.
